...
##事前分布の設定
alpha1 <- matrix(5.0, nrow=seg_u, ncol=seg_i)   #ユーザーセグメントのディクレリ事前分布
alpha2 <- matrix(5.0, nrow=seg_i, ncol=seg_u)   #アイテムセグメントのディクレリ事前分布
...
for(rp in 1:R){
  ...
  ##ユーザーのセグメント割当を生成
  y1 <- as.matrix(sparse_data %*% item_vec)   #アイテムセグメントごとの購買頻度
  LLi1 <- y1 %*% log(t(oldtheta))   #セグメントごとに多項分布の尤度を計算
  ...
  ##アイテムのセグメント割当を生成
  y2 <- as.matrix(sparse_data_T %*% user_vec)   #ユーザセグメントごとの購買頻度
  LLi2 <- y2 %*% t(log(oldphi))   #セグメントごとに多項分布の尤度を計算
